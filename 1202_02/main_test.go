...
func Test_reducedElementsList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%v => %v", tt.args, tt.want), func(t *testing.T) {
			if got := reducedElementsList(tt.args); !assert.ElementsMatch(t, got, tt.want) {
				t.Errorf("reducedElementsList() = %v, want %v", got, tt.want)
			}
		})
	}
}
